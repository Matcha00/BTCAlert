...
- 登录网关：`btc-vol-auth.service`，使用 Unix Socket、Apache htpasswd 哈希和安全 Cookie
...
未登录访问会跳转到 `/_login`。登录服务复用服务器现有用户名和 htpasswd
密码哈希，登录后 Cookie 默认有效 7 天；密码、哈希及 Cookie 签名密钥均不进入 Git。

```bash
systemctl status btc-vol-dashboard.service
systemctl status btc-vol-auth.service
...
tar -czf /root/btc-vol-alert-backup.tgz \
  ...
  /etc/systemd/system/btc-vol-auth.service \
  /etc/btc-vol-alert-dashboard.htpasswd \
...
systemctl enable --now btc-vol-alert.timer btc-vol-dashboard.service btc-vol-auth.service
...
```bash
systemctl status btc-vol-alert.service btc-vol-alert.timer btc-vol-dashboard.service btc-vol-auth.service
