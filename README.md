...
- 计算之前 52 周 open interest 均值。
- 当前 OI 大于 `52周均值 x 1.0` 时触发。
- 同一个 COT 报告日期只推送一次。
...
CFTC_BTC_OI_ABSOLUTE_THRESHOLD=
```
...
如果你希望“突破一个固定历史高位才报警”，设置绝对阈值：

```bash
CFTC_BTC_OI_ABSOLUTE_THRESHOLD=35000
```
